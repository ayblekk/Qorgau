...
/**
 * Core on-device analysis path (rules first). Classifier hook lands later.
...
class AnalyzeIncomingMessageUseCase(
    ...
    private suspend fun analyzeAndStore(
        ...
        // Stage 1: rules only path is always available.
        // Optional classifier will run later only when evaluation.isUncertain.
        val record = AnalysisRecord(
        ...
        val shouldAlert = evaluation.riskLevel == RiskLevel.HIGH ||
            (
                evaluation.riskLevel == RiskLevel.SUSPICIOUS &&
                    settings.sensitivity != Sensitivity.LOW
                )

        return Outcome(record = stored, shouldAlert = shouldAlert)
    }
}
